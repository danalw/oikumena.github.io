...
# NO dependency on third modules
...
my $file = {
  ...
  # HEADING template
  tt_heading  => catfile('html', 'tt', 'heading.tt.html'),
  # PLAYLIST template
  tt_playlist => catfile('html', 'tt', 'item.tt.html'),
  # VIDEO template
  tt_video    => catfile('html', 'tt', 'video.tt.html'),
  # HEADER html
  html_header => catfile('html', 'header.html'),
  # FOOTER html
  html_footer => catfile('html', 'footer.html'),
};

# RUN

&index(data($file->{data}));
...
sub index {
  ...
  for my $cursor (@$data) {
    ...
    # <!-- PLAYLIST -->
    my $html_course = template(read_file($file->{tt_playlist}), $cursor);
